...
void unenrollAStudentFromCourse();
...
void unenrollAStudentFromCourse() {
	...
	cout << " *** UnEnroll Student From Course ***" << endl;
	...
	if (backend.coursesLength > 0)
	{
		...
		cout << "Enter index of course you unenroll a student from: ";
		int selection = 0;
		cin >> selection;


		int count = 1;
		for (int i = 0; i < backend.currentCourses[selection - 1].studentsLength; i++)
		{
			cout << count << "1. " << backend.currentCourses[selection - 1].enrolledStudents[i].rollNumber << "   " << backend.currentCourses[i].enrolledStudents[i].person.name << endl;
		}

		cout << "Select the index of student you want to unenroll from this course: ";
		int stdSel;
		cin >> stdSel;

		backend.currentCourses[selection].enrolledStudents[stdSel - 1] = backend.currentCourses[selection].enrolledStudents[(stdSel - 1) + 1];

		cout << endl << "Student UnEnrolled ";

		cout << endl << "Enter any key and Enter to go back : ";
		char x;
		cin >> x;
		displayAdminPage();
	}
